...
class SuspendTicketForm extends StatefulWidget {
  const SuspendTicketForm({super.key, required this.ticket});
  final Map<String, dynamic> ticket;
  @override
  State<SuspendTicketForm> createState() => SuspendTicketFormState();
}

class SuspendTicketFormState extends State<SuspendTicketForm> {
  ...
  void _checkForDraft() async {
    final draft = await DraftManager.getSuspendDraft(widget.ticket['id']);
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('È stata trovata una bozza di sospensione non completata per questo ticket.'),
            SizedBox(height: 8),
            Text(
        ...
        actions: [
          TextButton(
            onPressed: () {
              DraftManager.deleteSuspendDraft(widget.ticket['id']);
  ...
  void _saveDraft() async {
    ...
    await DraftManager.saveSuspendDraft(widget.ticket['id'], draftData);
  }
  ...
  void getAnagrafica(valore) async {
    ...
    try {
      ...
      print('Errore getAnagrafica: $error');
      setState(() {
        _optionsAnagrafica = [];
      });
    } finally {
  ...
  _suspendTicket(Map<String, dynamic> ticket) async {
    ...
    try {
      var data = {
        ...
        'metodoPagamento': _selectedPaymentOption ?? 'Nessuno',
        'datiMacchina': datiMacchina,
        ...
        'iva': _iva,
        'codUnivoco': _codUnivocoController.text,
        'rifFurgone': _vansValue ?? '',
        'ragSocialeFirmatario': _signerController.text,
        'supplemento': _supplementoController.text.isEmpty
            ? '0'
            : _supplementoController.text,
      };

      print(data);
      int status = await TicketApi().suspendTicket(data, ticket['id']);

      CircularProgressIndicator(color: secondaryColor);
      if (status == 200) {
        // Elimina la bozza dopo la sospensione con successo
        await DraftManager.deleteSuspendDraft(ticket['id']);

        // ignore: use_build_context_synchronously
        FlutterPlatformAlert.showAlert(
          windowTitle: 'Ticket sospeso',
          text: 'Il ticket è stato correttamente sospeso',
          alertStyle: AlertButtonStyle.ok,
        ...
        FlutterPlatformAlert.showAlert(
          windowTitle: 'Si è verificato un errore',
          text: 'Il ticket non è stato correttamente sospeso',
          alertStyle: AlertButtonStyle.ok,
          iconStyle: IconStyle.error,
        );
        setState(() => _isLoading = false);
      ...
      print("ci ho provato");
      FlutterPlatformAlert.showAlert(
        windowTitle: "Errore nella sospensione del ticket",
        text:
            'Connessione al server non riuscita, controlla la connessione ad Internet e riprova.',
        alertStyle: AlertButtonStyle.ok,
        iconStyle: IconStyle.error,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.ticket['ragSocAzienda'] != null &&
      ...
      // Set a placeholder company object to indicate selection
      if (_selectedCompany == null) {
        _selectedCompany = {
          'ragSoc': widget.ticket['ragSocAzienda'],
          'indir': widget.ticket['indirizzo'] ?? '',
          'local': widget.ticket['citta'] ?? '',
          'partiva': widget.ticket['partiva'] ?? '',
          'codFisc': widget.ticket['codFisc'] ?? '',
          'tel': widget.ticket['numTel'] ?? '',
        };
      }
    }
            ...
            child: Column(
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: const Text('Sospendi Ticket'),
                ),
                // ElevatedButton(
                //   onPressed: () {
                //     if (_formKey.currentState!.validate()) {
                  ...
                  Flexible(
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child:
                            const Icon(Icons.add_rounded, color: primaryColor),
                      ),
                    ),
                  ),
                ],
      ...
      builder: (context, setModalState) {
        ...
        void filterCompanies(String query) {
          if (query.isEmpty) {
            setModalState(() {
              displayedCompanies =
                  List<Map<String, dynamic>>.from(_optionsAnagrafica);
            });
          } else {
            setModalState(() {
              displayedCompanies = _optionsAnagrafica
                  .where((company) {
                    ...
                    return ragSoc.contains(searchLower) ||
                        local.contains(searchLower);
                  })
                  .toList()
          ...
          debounceTimer = Timer(const Duration(milliseconds: 500), () async {
            ...
            try {
              ...
              setModalState(() {
                ...
                final existingIds = _optionsAnagrafica
                    .map((e) => e['id'])
                    .toSet();
                ...
                child: Column(
                  children: [
                    ...
                    TextField(
                      ...
                      autofocus: true,
                      decoration: InputDecoration(
                        hintText: 'Cerca per ragione sociale o città...',
                      ...
                      onChanged: (value) {
                        ...
                        if (value.length < 2) {
                          ...
                          // Search on server for better results
                              ...
                              children: [
                                ...
                                if (searchController.text.isNotEmpty) ...[
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () {
                                      setState(() {
                                        _ragSocController.text = searchController.text;
                                        _selectedCompany = null;
                                      });
                                      Navigator.pop(context);
                                    },
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                subtitle: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    if (company['local'] != null &&
                                        company['local'].toString().isNotEmpty)
                                      Text('📍 ${company['local']}'),
                                    if (company['partiva'] != null &&
                                        company['partiva']
                                            .toString()
                                            .isNotEmpty)
                                      Text('P.IVA: ${company['partiva']}')
                                    else if (company['codFisc'] != null &&
                                        company['codFisc']
                                            .toString()
                                            .isNotEmpty)
                                      Text('C.F: ${company['codFisc']}'),
                                  ],
                                ),
